...
PRODDIR:=/gpfs/data/snudem01lab/production/Demultiplexing
UPLOADSDIR:=/gpfs/data/molecpathlab/production/isg-uploads_v62/PROD
APIDIR:=/gpfs/data/snudem01lab/pipelines/bin/ArcherDx/archer_variantplex_api_jobsubmit.py
...
./nextflow:
	@[ -d "$(NXF_FRAMEWORK_DIR)" ] && $(MAKE) remove-framework || : \
 	module unload jdk && module load jdk/21.0.1 ; \
 	printf ">>> Installing Nextflow in the local directory\n" && \
 	wget -qO- https://github.com/nextflow-io/nextflow/releases/download/v24.04.2/nextflow-24.04.2-all | bash
...
# Change this to few options, compared to previous version
run-recurse:
	@echo "SYSTEM: $${SYSTEM}, SEQTYPE: $${SEQTYPE}" ; \
	if grep -q 'bigpurple' <<<"$${SYSTEM}" && grep -q 'PACT' <<<"$${SEQTYPE}" ; then echo ">>> Running run-PACT-bigpurple"; $(MAKE) run-PACT-bigpurple fix-permissions fix-group ; \
...
RUNDIR:=$(shell python -c 'import json; print( json.load(open("$(CONFIG_OUTPUT)")).get("runDir", "") )')
SAMPLESHEET:=$(shell python -c 'import json; print( json.load(open("$(CONFIG_OUTPUT)")).get("samplesheet", "") )')

run-PACT-bigpurple: install
	module unload jdk && module load jdk/21.0.1 ; \
	./nextflow run main.nf $(RESUME) -with-notification -with-timeline -with-trace -with-report -profile bigpurple,PACT --runDir $(RUNDIR) --samplesheet $(SAMPLESHEET)


run-Archer-bigpurple: install
	module unload jdk && module load jdk/21.0.1 ; \
	./nextflow run main.nf $(RESUME) -with-notification -with-timeline -with-trace -with-report -profile bigpurple,Archer --runDir $(RUNDIR) --samplesheet $(SAMPLESHEET)
...
submit:
	...
	if grep -q 'bigpurple' <<<'$(HOSTNAME)'; then echo ">>> Running submit-bigpurple"; $(MAKE) submit-bigpurple ; \
...
# e.g. Nextflow running in a sbatch job on a compute node
...
# ~~~~~ DEPLOY NGS607 ~~~~~ (need to be updated based on new pipeline workflow for dsl2) comment out for now #
